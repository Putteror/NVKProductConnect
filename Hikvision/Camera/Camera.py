...
class Camera:
	...
	def update_device_information(self):
		...
		api_url = self.hostAddress + "/ISAPI/System/Video/inputs/channels/1"
		...
		payload = """ 
				...
				<name>ShowroomCamPutterNaja</name>
				<videoFormat>PAL</videoFormat>
				</VideoInputChannel>
									
				"""

		response = requests.put(url=api_url, data=payload, auth=self.auth)
...
if __name__ == '__main__':
	...
	camera = Camera(hostAddress='http://192.168.26.179', username='admin', password='nVk12345')
	# print(camera.get_device_protocal_capability())
	# print(camera.get_device_information())
	print(camera.update_device_information())
